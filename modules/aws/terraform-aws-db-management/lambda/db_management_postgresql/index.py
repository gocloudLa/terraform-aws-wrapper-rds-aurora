...
def update_users_and_grants(data):
    ...
    for user_info in data['users']:
        ...
        for grant_info in grants:
            privileges = grant_info['privileges']
            schema = grant_info['schema']
            database = grant_info['database']
            ...
            if conn:
                cursor = conn.cursor()
                if table == '*':
                    grant_query = f"GRANT {privileges} ON ALL TABLES IN SCHEMA {schema} TO {username}"
                elif table :
                    grant_query = f"GRANT {privileges} ON TABLE {schema}.{table} TO {username}"
                else:
                    grant_query = f"GRANT {privileges} ON DATABASE {database} TO {username}"
